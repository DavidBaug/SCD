...
class ej1l_Buffer extends AbstractMonitor
{
    private int numSlots = 0;							// Tamanio del array buffer
    private int cont = 0;								// Contador para comparar el numero de posiciones en el array en las hebras
    ...
    public ej1l_Buffer(int p_numSlots){		// Constructor del Buffer (Inicializamos el array)
        numSlots = p_numSlots;
...
class ej1l_Productor implements Runnable
{
    private ej1l_Buffer bb;			// Creamos objeto de la clase Buffer 
    ...
    public ej1l_Productor(ej1l_Buffer pbb, int pveces, int pnumP){	// Constructor de la clase
        bb    = pbb;
        veces = pveces;
        numP  = pnumP;
...
class ej1l_Consumidor implements Runnable
{
    private ej1l_Buffer bb;
    int veces;
    int numC;
    Thread thr;
    public ej1l_Consumidor(ej1l_Buffer pbb, int pveces, int pnumC)
    {
        bb    = pbb;
        veces = pveces;
        numC  = pnumC;
...
class ej1_pcl
{
    public static void main(String[] args)
    {
        ...
        ej1l_Productor[]  prod = new ej1l_Productor[Integer.parseInt(args[0])];       // Numero productores
        int iter_prod = Integer.parseInt(args[1]);                                  // Iteraciones productor

        ej1l_Consumidor[] cons = new ej1l_Consumidor[Integer.parseInt(args[2])];		// Numero consumidores  
        int iter_cons = Integer.parseInt(args[3]);                                  // Iteraciones consumidor
        
        ej1l_Buffer buffer = new ej1l_Buffer(Integer.parseInt(args[4]));				// Tamaño buffer
        ...
        for (int i = 0; i < cons.length; i++)
            cons[i] = new ej1l_Consumidor(buffer,iter_cons,i);

        for (int i = 0; i < prod.length; i++)
            prod[i] = new ej1l_Productor(buffer,iter_prod,i);
