...
{ 
  ...
  { 
    ...
    { 
      ...
      Hebra[] vc = new Hebra[3] ; 
      
      // calcula e imprime el resultado usando dos hebras contadoras
      
      vc[0] = new Hebra( "Contador3 ", 3, contH);
      vc[1] = new Hebra( "Contador5 ", 5, contH);
      vc[2] = new Hebra( "Contador7 ", 7, contH);
      
      
      vc[0].thr.start(); vc[1].thr.start(); vc[2].thr.start(); // lanza hebras
      vc[0].thr.join();  vc[1].thr.join(); vc[2].thr.join();  // espera terminación
      ...
      for ( int i = 1 ; i <= 1000 ; i++ )
      { 
        if ( i % 5 == 0 ) contV = contV + 1 ;
        if ( i % 3 == 0 ) contV = contV + 1 ;
        if ( i % 7 == 0 ) contV = contV + 1 ;
      }
